...
open class KotlinApiBuildTask @Inject constructor(
    ...
    fun generate() {
        ...
        val filteredSignatures = signatures
            .filterOutNonPublic(
                nonPublicPackages = ignoredPackages, nonPublicClasses = ignoredClasses,
                publicPackages = publicPackages, publicClasses = publicClasses
            )
            .filterOutAnnotated(nonPublicMarkers.map { it.replace(".", "/") }.toSet())
            .filterOutNotAnnotated(publicMarkers.map { it.replace(".", "/") }.toSet())
